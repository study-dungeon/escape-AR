...
THREEx.ArMarkerControls = function (context, object3d, parameters) {
  ...
  this.parameters = {
    ...
    changeMatrixMode: parameters.changeMatrixMode !== undefined ?
      parameters.changeMatrixMode : 'modelViewMatrix',
  };
...
THREEx.ArMarkerControls.prototype._postInit = function () {
  ...
  arController.addEventListener('getMarker', function (event) {
    const markerName = _this.parameters.patternUrl.split('/').pop().split('.')[0]
    if (
      event.data.type === artoolkit.PATTERN_MARKER &&
      _this.parameters.type === 'pattern'
    ) {
      if (_this.markerId === null) return;
      if (event.data.marker.idPatt === _this.markerId)
      ...
      if (_this.markerId === null) return;
      if (event.data.marker.idMatrix === _this.markerId)
        onMarkerFound(markerName);
    }
    else if (
    ...
    function onMarkerFound(markerName) {
      ...
      var tmpMatrix = new THREE.Matrix4().copy(
        _this.context._projectionAxisTransformMatrix
      );
      ...
      // dispatchEvent
      _this.dispatchEvent({ type: markerName });
    }
  });
};
